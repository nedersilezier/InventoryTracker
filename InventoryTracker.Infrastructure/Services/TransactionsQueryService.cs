...
namespace InventoryTracker.Infrastructure.Services
{
    ...
    //TO DO: introduce mappers, remove duplicate code
    public class TransactionsQueryService: ITransactionsQueryService
    {
        ...
        public async Task<PagedResult<TransactionListDTO>> GetCurrentUsersDraftsAsync(GetUsersDraftsParameters parameters, CancellationToken cancellationToken)
        {
            var query = _context.Transactions
                .Where(t => t.CreatedBy == parameters.UserName && t.Status == TransactionStatus.Draft)
                .AsNoTracking()
                .Include(t => t.Client)
                .AsQueryable();
            var selectedTypes = new List<TransactionType>();
